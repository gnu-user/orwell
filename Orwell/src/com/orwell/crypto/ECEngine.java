...
public class ECEngine
{
	 ...
	 * parameters.
	 * 
	 * NOTE: reusing an IV completely destroys security, ensure that the nonce you
	 * use is unique and is generated by a CSPRNG.
     * 
	 * ECEngine is created using the following default parameters:
	 *  - SIC/CTR block cipher mode using AESEngine with 128-bit block size
	 *  - ECDHCBasicAgreement key agreement
	 *  - KDF2BytesGenerator Key derivative function
	 *  - SHA256 Digest/HMAC
	 * 
	 * @param nonce the cryptographic nonce which is used to generate IVs
	...
	public ECEngine(CipherParameters nonce, APrioriInfo sharedInfo)
	{
		...
		//this.cipher = new CTSBlockCipher(new CBCBlockCipher(new AESEngine()));
     ...
     * @param nonce the cryptographic nonce which is used to generate IVs, can be
     * null if the block cipher mode does not require an IV
     * @param sharedInfo the shared information exchanged by users a priori
     * 
